...
```python
data = f.read()
data = f.read([maxbytes]) # Leer archivos con un tamaño máximo de caracteres 
```
...
Los archivos deben ser cerrados adecuadamente y es un paso muy fácil que no debe olvidarse. Aunque preferiblemente se debe utilizar la sentencia `with`. 
