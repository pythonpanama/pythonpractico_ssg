...
```python
>>> rs = r'c:\datosnuevos\ejercicio' # Sin formato (barra invertida no interpretada)
>>> rs
'c:\\datosnuevos\\ejercicio'
```
...
s.sort()                    # [1, 3, 7, 10]
 ...
 # Orden de atrás para adelante
s = [10, 1, 7, 3]
s.sort(reverse=True)        # [10, 7, 3, 1]

# Funciona con cualquier tipo de datos en orden.
s = ['foo', 'bar', 'spam']
s.sort()                    # ['bar', 'foo', 'spam']
```
...
*Cuidado: Las listas no son diseñadas para operaciones matemáticas*
...
```python
>>> simboloslista[-2:] = misimb
>>> simboloslista
['HPQ', 'AAPL', 'AIG', 'MSFT', 'YHOO', 'GOOG']
>>>
```

Cuando hace esto, la lista en el lado izquierdo  (`simboloslista`) será reajustada apropiadamente para que el lado derecho (`misimb`) ocupe un puesto en ella.
...
```python
>>> # Está 'AIG' en `simboloslista`?
True
>>> # Está 'AA' en `simboloslista`?
False
>>> # 'CAT' no esta en `simboloslista`?
True
>>>
```
...
```python
>>> simboloslista.sort(reverse=True)
>>> simboloslista
...
```python
>>> a = ','.join(simboloslista)
>>> a
'YHOO,RHT,HPQ,GOOG,AIG,AAPL,AA'
>>> b = ':'.join(simboloslista)
>>> b
'YHOO:RHT:HPQ:GOOG:AIG:AAPL:AA'
>>> c = ''.join(simboloslista)
>>> c
...
```python
>>> nums = [101, 102, 103]
>>> articulo = ['spam', simboloslista, nums]
>>> articulo
...
```python
f = open('foo.txt', 'rt')     # Disponible para lectura (texto)
g = open('bar.txt', 'wt')     # Disponible para escritura (texto)
```
...
```python
data = f.read()
data = f.read([maxbytes]) # Leer archivos con un tamaño máximo de bytes
```
...
Los archivos deben ser cerrados adecuadamente y es un paso que mucha gente suele olvidar. Aunque preferiblemente se debe utilizar la sentencia `with`. 
