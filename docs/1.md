...
Si no ha utilizado el shell o una terminal, probablemente debería terminar un breve tutorial sobre eso primero y luego regresar.

Aunque hay muchos entornos en donde puede programar en Python, será un programador más sólido de Python si puede ejecutar, depurar e interactuar con Python en la terminal. Este es el entorno nativo de Python. Si puede usar Python aquí, podrá usarlo en cualquier otro lugar.
...
Utilice el comando `help()` para obtener ayuda sobre la función abs (). Luego use `help()` para obtener ayuda sobre la función round (). Escriba `help()` solo sin valor para ingresar al visor de ayuda interactiva.
...
> Una mañana, sales y colocas un billete de un dólar en la acera de la torre Sears en Chicago. Cada día a partir de entonces, sale el doble de facturas. ¿Cuánto tiempo tarda la pila de billetes en superar la altura de la torre?
...
### 1.2.12: Sangrías (las de texto, no las de vino)
...
Este es el primer conjunto de ejercicios en los que necesita crear archivos Python y ejecutarlos. A partir de este punto, se asume que está editando archivos en el directorio `practico-python/Work/`. Para ayudarle a localizar el lugar adecuado, se han creado varios archivos de inicio vacíos con los nombres de archivo adecuados. Busque el archivo `Work/bounce.py` que se utilizó en el primer ejercicio.
...
Los flotantes se representan con precisión doble utilizando la representación de CPU nativa [IEEE 754] (https://en.wikipedia.org/wiki/IEEE_754).
Es lo mismo que el tipo "doble" en el lenguaje de programación C.
...
Recordatorio: Estos ejercicios asumen que está trabajando en el directorio `/Work`. Busque el archivo `mortgage.py`.
...
#### 1.19: Extracción y reasignación de elementos
...
#### 1.22: Añadiendo, insertando y borrando elementos
...
#### 1.24: Ajuntando todo nuevamente
...
### 1.6.1: I/O de un Archivo (entrada / salida)
...
Se supone que está trabajando en el archivo `practice-python/Work/` directorio. Si no está seguro, puede averiguar dónde Python se está ejecutando haciendo esto:

```python
>>> import os
>>> os.getcwd()
'/Users/beazley/Desktop/practical-python/Work' # Su salida será diferente
>>>
```
...
#### 1.30: Convirtiendo una secuencia de comandos en una función

Tome el código que escribió para el programa `pcost.py` en [Ejercicio 1.27] (06_Files.md) y conviértalo en una función `cartera_costo(nombre_de_archivo)`. La función toma un nombre de archivo como entrada, lee los datos de la cartera en ese archivo y devuelve el costo total de la cartera como flotante.
...
#### 1.31: Administración de errores
...
#### 1.32: Usando una función de biblioteca local
...
#### 1.33: Leyendo desde la línea de comando
